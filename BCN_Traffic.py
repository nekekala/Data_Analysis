...
#Using a defining methods aproach, so we define our Methods
def concat (indir):
    ...
    for filename in filelist:
        df = pd.read_csv(filename, low_memory=False) #read each file...
        avg = df['estatActual'].mean() #...find the average of the real traffic...
...
def graph(average, months): #Plot the results
        ...
        plt.bar(months, average, align='center',
                color=['olive', 'yellowgreen', 'lawngreen', 'lightgreen', 'g', 'mediumseagreen', 'mediumaquamarine', 'mediumturquoise', 'darkcyan',
                       'royalblue', 'navy'])
...
def main():
    ...
    average, months = concat('G:\Akiss\Empleo\StarLab')
